...
# 脚本位于 debug/，同时加入 src/ 和 train/ 以便导入项目模块。
_CUR = os.path.dirname(os.path.abspath(__file__))
_SRC = os.path.abspath(os.path.join(_CUR, "../src"))
_TRAIN = os.path.abspath(os.path.join(_CUR, "../train"))
for _p in (_SRC, _TRAIN, _CUR):
...
def check_push_point(ctx):
    ...
    light_yellow_rgb = np.array([255, 235, 140], dtype=np.float32)
    light_green_rgb = np.array([144, 238, 144], dtype=np.float32)
    yellow_alpha = 0.32
    green_alpha = 0.50
    ...
    for step_idx, action_idx in enumerate(ctx.args.action):
        ...
        for env_idx, state in enumerate(scene.states):
            ...
            neighborhood_mask = np.asarray(
                debug_info.get(
                    "edge_neighborhood_mask",
            ...
            candidate_mask = np.asarray(
                debug_info.get(
                    "candidate_mask", np.zeros(base_rgb.shape[:2], dtype=bool)
                ),
                dtype=bool,
            )
            overlay_float = base_rgb.astype(np.float32)
            overlay_float[neighborhood_mask] = (
                (1.0 - yellow_alpha) * overlay_float[neighborhood_mask]
                + yellow_alpha * light_yellow_rgb
            )
            overlay_float[candidate_mask] = (
                (1.0 - green_alpha) * overlay_float[candidate_mask]
                + green_alpha * light_green_rgb
            )
            ...
            if geometry_valid and push_pixel is not None:
                cv2.circle(overlay, tuple(push_pixel), 4, (255, 0, 0), thickness=-1)
            else:
                cv2.putText(
                    overlay,
                    f"NO VALID PUSH: {debug_info.get('geometry_invalid_reason', 'unknown')}",
            ...
            print(
                f"  候选像素={int(np.count_nonzero(candidate_mask))}, "
                f"30mm邻域像素={int(np.count_nonzero(neighborhood_mask))}, "
