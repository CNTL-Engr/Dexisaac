...
class DQNAgent:
    ...
    def train_step(self, batch_size=32):
        ...
        # [关键] 立即释放 states，因为后续不再需要
        del states
        torch.cuda.empty_cache()
        
        # 阶段2: 计算目标Q值（Double DQN）
        with torch.amp.autocast('cuda'):
            with torch.no_grad():
                # Double DQN: 用 policy net 选择 action
                q_next_policy = self.policy_net(next_states)  # (B, 8)
                next_actions = q_next_policy.max(dim=1)[1]  # (B,)
        ...
        # [内存优化] 清理剩余tensor (states已在前面释放)
        del q_pred, q_values, actions, rewards, dones, loss
        torch.cuda.empty_cache()
