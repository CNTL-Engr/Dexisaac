...
class PushEnv:
    ...
    def __init__(self, scene, args):
        ...
        # [新增] 场景随机化和力监测开关
        self.force_scene_regenerate = getattr(args, 'force_scene_regenerate', False)
        self.enable_contact_sensors = getattr(args, 'enable_contact_sensors', False)
        
        # [新增] 崩飞力信息日志列表（用于写入CSV）
        self.explosion_logs = []
    ...
    def reset(self, env_indices=None):
        ...
        if self.spawned_objects is not None:
            ...
            self.spawned_objects = None
            # 清理旧的接触传感器缓存（物体 prim 已重建）
            self.scene.clear_contact_sensors()
        
        # 重置 scene
        spawned_objects = self.scene.create_clutter_environment(
            num_objects_range=(self.num_objects_min, self.num_objects_max),
            force_regenerate=self.force_scene_regenerate,
            enable_contact_sensors=self.enable_contact_sensors
        )
                ...
                if max_exceed > explode_threshold:
                    if obj_env_id not in exploded_envs:
                        ...
                        if self.enable_contact_sensors:
                            # 确定主要超出的轴
                            if x_exceed >= y_exceed and x_exceed >= z_exceed:
                                axis = 'X'
                                exceed_dist = x_exceed
                            elif y_exceed >= z_exceed:
                                axis = 'Y'
                                exceed_dist = y_exceed
                            else:
                                axis = 'Z'
                                exceed_dist = z_exceed
                            self._log_explosion_force_info(obj, obj_env_id, obj_name, exceed_dist, axis)
    ...
    def _check_out_of_bounds(self, env_idx, spawned_objects):
        ...
        for obj in spawned_objects:
            ...
            if not x_in:
                ...
                # [新增] 崩飞时记录力信息
                if is_exploded and self.enable_contact_sensors:
                    self._log_explosion_force_info(obj, env_idx, obj_name, x_exceed, 'X')
            ...
            if not y_in:
                ...
                # [新增] 崩飞时记录力信息
                if is_exploded and self.enable_contact_sensors:
                    self._log_explosion_force_info(obj, env_idx, obj_name, y_exceed, 'Y')
            ...
            if not z_in:
                ...
                # [新增] 崩飞时记录力信息
                if is_exploded and self.enable_contact_sensors:
                    self._log_explosion_force_info(obj, env_idx, obj_name, z_exceed, 'Z')
    ...
    def _log_explosion_force_info(self, exploded_obj, env_idx, obj_name, exceed_distance, axis):
        """
        [功能]: 记录导致物体崩飞的力信息（使用 ContactSensor）
        [输入]: exploded_obj - 崩飞的物体, env_idx - 环境索引, obj_name - 物体名, exceed_distance - 超出距离, axis - 超出轴
        """
        ...
        try:
            force_info = {
                ...
                'env_idx': env_idx,
                'obj_name': obj_name,
                'exceed_distance': round(exceed_distance, 4),
                'axis': axis,
            ...
            # 获取物体的 prim_path
            prim_path = exploded_obj.cfg.prim_path
            
            # 使用 ContactSensor 获取净接触力
            contact_info = self.scene.get_contact_force_info(prim_path)
            
            if contact_info.get('error'):
                force_info['error'] = contact_info['error']
                print(f"  ⚠️ [崩飞力监测] 获取力信息失败: {contact_info['error']}")
            else:
                force_info['net_force_magnitude'] = contact_info.get('net_force_magnitude')
                
                net_force = force_info['net_force_magnitude']
                if net_force is not None and net_force > 0.01:
                    print(f"  💥 [崩飞力监测] Env {env_idx} | 物体: {obj_name}")
                    print(f"     超出{axis}轴: {exceed_distance:.2f}m | 净接触力: {net_force:.2f} N")
                else:
                    print(f"  💥 [崩飞力监测] Env {env_idx} | 物体: {obj_name}")
                    print(f"     超出{axis}轴: {exceed_distance:.2f}m | 净接触力: 0 N (物体已脱离接触)")
            
            # 添加到日志列表
            self.explosion_logs.append(force_info)
            
        except Exception as e:
            print(f"  ⚠️ [崩飞力监测] 记录失败: {e}")
